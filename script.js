...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ouvinte de evento para o botão "Jogar Novamente"
    ...
    // Ouvinte de evento para o botão "Mudar de Posição"
    document.querySelector('.set-pos').addEventListener('click', () => {
        const fen = prompt("Enter the FEN notation for the desired position!");
        if(fen !== null){
            if(game.load(fen)){
                board.position(fen);
                moveHistory.tectContent = '';
                moveCount = 1;
                userColor = 'w';
            }else{
                alert("Invaid FEN notation. Please try again.");
          }
        }
    });

    //Ouvinte de evento para o botão "Virar Tabuleiro"
